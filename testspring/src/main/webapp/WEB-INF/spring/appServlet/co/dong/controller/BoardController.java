...
public class BoardController {
	...
	@RequestMapping(value="board/login", method= RequestMethod.POST) // dispathcer servlet
	public String login(@RequestParam Map<String,Object> map, // 어떤 함수
			HttpServletRequest request, HttpServletResponse response,HttpSession session) throws Exception {
		request.setCharacterEncoding("utf-8"); // 어떤 행동
		logger.info("요청정보 받아서 보내기");
		Map user = boardService.login(map);
		
		if(user == null) { //로그인 실패 // view resolver
			logger.info("로그인 안됨");
